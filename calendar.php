...
class calendar extends rcube_plugin
{
    ...
    public function startup($args)
    {
        ...
        if ($args['task'] == 'calendar' && $args['action'] != 'save-pref') {
            ...
            $this->register_action('google-oauth-start', [$this, 'google_oauth_start']);
            $this->register_action('google-oauth-callback', [$this, 'google_oauth_callback']);
            $this->register_action('google-oauth-disconnect', [$this, 'google_oauth_disconnect']);
    ...
    public function preferences_list($p)
    {
        ...
        // --- Google OAuth: connect/disconnect + calendars selection ---
        $this->load_google_driver_class();
        if (class_exists('google_driver')) {
            $gdrv = new google_driver($this);
            $prefs = (array)$this->rc->user->get_prefs();
            $selected = isset($prefs['calendar_google_selected']) ? (array)$prefs['calendar_google_selected'] : [];

            $p['blocks']['google']['name'] = 'Google';
            if (empty($p['current'])) {
                $p['blocks']['google']['content'] = true;
                return $p;
            }

            $content = new html_table(['cols' => 2, 'class' => 'propform']);
            $authurl = method_exists($gdrv, 'oauth_auth_url') ? $gdrv->oauth_auth_url() : null;

            if ($authurl) {
                // Assume connected if there is a token
                $connected = !empty($gdrv->list_calendars());
                $btn_label = $connected ? $this->gettext('disconnect') : $this->gettext('connect');
                $btn_action = $connected ? 'google-oauth-disconnect' : 'google-oauth-start';

                $button = html::tag('a', ['class' => 'button', 'href' => $this->rc->url(['_task'=>'calendar','_action'=>"plugin.$btn_action"])], rcube::Q($btn_label . ' Google'));
                $content->add('title', rcube::Q('Account'));
                $content->add('', $button);

                // Calendars list with checkboxes
                $calendars = $gdrv->list_calendars();
                if (!empty($calendars)) {
                    $fieldset = '<div>';
                    foreach ($calendars as $id => $c) {
                        $id_attr = 'gcal_' . md5($id);
                        $checked = empty($selected) ? ' checked' : (in_array($id, $selected) ? ' checked' : '');
                        $fieldset .= '<div><label for="'. $id_attr .'">'
                                  . '<input type="checkbox" id="'. $id_attr .'" name="_calendar_google_selected[]" value="'. rcube::Q($id) .'"'. $checked .'/>'
                                  . rcube::Q($c['name']) . '</label></div>';
                    }
                    $fieldset .= '</div>';

                    $content->add('title', rcube::Q('Calendars'));
                    $content->add('', $fieldset);
                }
            } else {
                $content->add('title', rcube::Q('Google API'));
                $content->add('', rcube::Q('google/apiclient missing. Run composer in plugins/calendar and set Client ID/Secret in config.inc.php.'));
            }

            $p['blocks']['google']['options']['google'] = [
                'title' => rcube::Q('Google Calendar'),
...
$("#rcmfd_new_category").keypress(function(event) {
    ...
    {
        $this->load_google_driver_class();
        $g = new google_driver($this);

        $url = $g->oauth_auth_url();
        if (!$url) {
            $this->rc->output->show_message('calendar.calendar', 'error', 'Google API client not installed. Run composer install in plugins/calendar/');
            return;
        }
        header('Location: ' . $url);
        exit;
    ...
    public function google_oauth_callback()
    {
        $this->load_google_driver_class();
        $g = new google_driver($this);
        $res = $g->oauth_handle_callback();

        if ($res['ok']) {
            $this->rc->output->show_message('calendar.successadd', 'confirmation', ['name' => 'Google: ' . $res['email']]);
        } else {
            $this->rc->output->show_message('calendar.error', 'error', 'Google OAuth failed: ' . $res['error']);
        }

        // redirect back to preferences
        $this->rc->output->redirect(array('_task' => 'settings', '_action' => 'preferences', '_section' => 'calendar'));
    }

    /**
     * Disconnect Google account
     */
    public function google_oauth_disconnect()
    {
        $this->load_google_driver_class();
        $g = new google_driver($this);
        $g->revoke_user_token();
        $this->rc->output->show_message('calendar.success', 'confirmation', 'Google account disconnected.');
        $this->rc->output->redirect(array('_task' => 'settings', '_action' => 'preferences', '_section' => 'calendar'));
    }

    protected function load_google_driver_class()
    {
        $file = $this->home . '/drivers/google/google_driver.php';
        if (is_readable($file)) {
            require_once $this->home . '/drivers/calendar_driver.php';
            require_once $file;
        }
